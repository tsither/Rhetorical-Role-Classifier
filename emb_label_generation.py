...
def main():
    ...
    train_data = Dataset_Reader(TRAIN_DATA_PATH)
    test_data = Dataset_Reader(TEST_DATA_PATH)
    ...
    for idx, train_idx in enumerate(train_doc_idxs):
        TRAIN_emb, TRAIN_labels = data_to_embeddings(train_idx, train_batched_texts[idx], train_batched_labels[idx],
                                                    ...
                                                    emb_model=emb_model)
        legal_train_emb, legal_train_labels = data_to_embeddings(train_idx, train_batched_texts[idx],
                                                                train_batched_labels[idx],label_encoder,
                                                                max_length_dict_TRAIN, tokenizer=legal_tokenizer,
                                                                emb_model=legal_emb_model)
    ...
    for idx, test_idx in enumerate(test_doc_idxs):
        TEST_emb, TEST_labels = data_to_embeddings(test_idx, test_batched_texts[idx], test_batched_labels[idx],
                                                ...
                                                emb_model=emb_model)
        legal_test_emb, legal_test_labels = data_to_embeddings(test_idx, test_batched_texts[idx],
                                                            test_batched_labels[idx],label_encoder,
                                                            max_length_dict_TEST, tokenizer=legal_tokenizer,
                                                            emb_model=legal_emb_model)
